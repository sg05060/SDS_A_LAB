...
// 몇 bit로 선언하는것이 알맞을까?
    localparam ADD_WDITH =          ;
    localparam MUL_WDITH =          ;
    wire [ : ] add_result = a + b   ;
    wire [ : ] mul_result = a * b   ;
...
// fill your code here
    parameter DATA_WIDTH   =           ;  // data의 width 와 관련된 parameter
    parameter MEM_DEPTH    =           ;  // data의 총 개수와 관련된 parameter

    localparam ADDR_WIDTH  = #clog2( ) ;  // fill blank ( )
    reg [ : ] memory [ : ]             ;  // memory 선언
//-------------------------------------
