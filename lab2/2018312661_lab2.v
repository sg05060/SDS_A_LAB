...
// a와 b의 합은 180으로 8비트로 충분히 표현 가능하다
    localparam MUL_WDITH =    10      ;
// a와 b의 곱은 875으로 10비트로 충분히 표현 가능하다

    wire [ADD_WDITH-1:0] add_result = a + b   ;
    wire [MUL_WDITH-1:0] mul_result = a * b   ;
...
// fill your code here
    ...
    reg [DATA_WIDTH-1:0] memory [MEM_DEPTH-1:0]             ;    // memory 선언
//-------------------------------------
...
// 이때, localparam 을 활용한 Indexed part-select 문법을 사용하여라.
    localparam sel = 8;
    vector [16+:sel];
// array는 한칸이 8비트인 4*16행렬, vector는 한 칸을 의미하는 8비트로
// indexed part-select문법을 사용하면 1행 1열은 [0+:sel] --> 0자리부터 8개
// 1행 2열은 0-7까지 했으니 [8+:sel] 이므로 8자리부터 8개
//-------------------------------------
